...
const TYPE_NAMES = {
  ...
  jazzmaster:"Fender Jazzmaster", prs:"PRS Custom 24", sg:"Gibson SG",
  dane:"Danelectro", mustang:"Fender Mustang", pacifica:"Yamaha Pacifica",
...
const HIDDEN_FROM_MAIN = {
  es335:"casino", pacifica:"yamahasg", lespaul:"lpjr", flyingv:"explorer",
  strat:"casino", tele:"lpjr", ricken:"casino", ibanez:"explorer",
  jazzmaster:"explorer", prs:"yamahasg", sg:"lpjr", dane:"casino",
};
...
function decideHiddenFromTies(tops){
  ...
  tops.forEach(k=>{ const hid=HIDDEN_FROM_MAIN[k]; if(hid) tally[hid]++; });
...
// ===== iOSの自動再生ガード解除（必要な場合のみ）=====
(function(){
  if (!SOUND_ON) return;
  let armed=false;
  const arm=()=>{
    if(armed) return; armed=true;
    ['sfx-pop','sfx-yeah'].forEach(id=>{
      const el=document.getElementById(id);
      if(el){ el.muted=true; el.play().catch(()=>{}).finally(()=>{ el.pause(); el.currentTime=0; el.muted=false; }); }
    });
